...
export class Select implements ISelect {
  ...
  From(): Select {
    if (this.Query.FromClause.BaseTable != null)
      throw new InvalidQueryError("InvalidQueryError", "BaseTable уже определена");
  ...
  Join(): Select {
    ...
    if (arguments.length >= 4 && arguments[3] instanceof Logic) {
      where = arguments[3];
    }

    if (!Utils.IsEmpty(rightTableName)) {
      this.Query.FromClause.Join(JoinType.Left, this.Query.FromClause.BaseTable, From.Table(rightTableName).Term, joinCond["Condition"]);
    } else {
  ...
  OrderBy(): Select {
    ...
    this.Query.OrderByTerms.push(new OrderByTerm(field, Utils.ConvertOrderByDir(dir), !Utils.IsEmpty(table) ? table.Term : null));
    return this;
  }
  ...
  GroupBy(): Select {
    ...
    this.Query.GroupByTerms.push(new GroupByTerm(field, Utils.IsEmpty(table) ? null : table.Term));
    return this;
  }
